...
export default function ProfilePage() {
  ...
  const displayName = email ? email.split('@')[0] : phone ? 'Driver' : 'Guest';
  const formattedPhone = phone ?? '+234 XXX XXX XXXX';
  const formattedEmail = email ?? 'No email on file';
    ...
    <div className="bg-white min-h-screen -mx-4 -mt-4 md:-mt-8 flex flex-col pb-24">
      ...
      <div className="px-6 py-10 flex flex-col items-center">
        ...
        <div className="text-center space-y-1 mb-8">
          <h2 className="text-3xl font-black text-slate-900 tracking-tight capitalize">{displayName}</h2>
          <p className="text-slate-500 font-semibold text-lg">{formattedPhone}</p>
          <p className="text-slate-400">{formattedEmail}</p>
        </div>
